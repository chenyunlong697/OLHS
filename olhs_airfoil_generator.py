...
def plot_airfoils(out_path: Path, x: np.ndarray, surfaces: List[Tuple[np.ndarray, np.ndarray]]) -> None:
    ...
    for i, (yu, yl) in enumerate(surfaces):
        color = cmap(float(i) / max(1, len(surfaces) - 1))
        ax.plot(x, yu, color=color, lw=0.9, alpha=0.9)
        ax.plot(x, yl, color=color, lw=0.9, alpha=0.9)
    ax.set_aspect("equal", adjustable="box")
    ax.set_xlabel("x/c")
    ax.set_ylabel("y/c")
    ax.set_title("OLHS-generated CST airfoils")
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    for idx, target in enumerate(targets):
        ...
        print(f"  - sample {idx:03d} solved (residual {err:.3e}) [{status}]" + note)
